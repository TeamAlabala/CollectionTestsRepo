...
namespace CollectionsTests
{
    public class CollectionsTests
    {
        ...
        {
            //Arrange
            var myValues = new Collection<int>(new int[1000000]);
        ...
        {
            //Arrange
            var myValues = new Collection<int>(5, 10, 15, 20, 25);

            //Act
            myValues.Add(30);

            //Assert
            var expected = new Collection<int>(5, 10, 15, 20, 25, 30);
        ...
        public void Test_Collections_RemoveAll()
        {
            ...
            //Assert
            var expected = new Collection<int>();
            Assert.AreEqual(expected.ToString(), myValues.ToString(), $"The new collection is:{myValues.ToString()}");
            Assert.That(myValues.Count == 0);
            Trace.WriteLine($"The new collection is:{myValues.ToString()}");
        }

        [TestCase("Geri",0, "Geri")]
        [TestCase("Geri, Nikolay", 1, "Nikolay")]
        public void Test_Collections_GetByInvalidIndex(string data,int index,string expectedValue)
        {
            //Arrange
            var myValues = new Collection<string>(data.Split(", "));

            //Act
            var actual = myValues[index];
